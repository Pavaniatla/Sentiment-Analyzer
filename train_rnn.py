...
def train():
    ...
    model = create_model(input_len=100, vocab_size=vocab_size, model_type='RNN', num_classes=5)
    stop = EarlyStopping(monitor='val_loss', patience=2, restore_best_weights=True)
    history = model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=5, batch_size=32, callbacks=[stop])

    os.makedirs("models", exist_ok=True)
    model.save_weights("models/RNN_weights.weights.h5")
    print("\u2705 RNN model weights saved.")

    with open("models/RNN_tokenizer.pkl", "wb") as f:
        pickle.dump(tokenizer, f)
    with open("models/RNN_label_encoder.pkl", "wb") as f:
        pickle.dump(label_encoder, f)
    with open("models/RNN_vocab_size.pkl", "wb") as f:
