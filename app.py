# ✅ app.py (Flask App for Sentiment Prediction)
...
def load_model_and_resources(model_type='RNN', input_length=100, num_classes=5):
    # Load tokenizer, label encoder, vocab size
    with open(f'models/{model_type}_tokenizer.pkl', 'rb') as f:
        tokenizer = pickle.load(f)
    with open(f'models/{model_type}_label_encoder.pkl', 'rb') as f:
        label_encoder = pickle.load(f)
    with open(f'models/{model_type}_vocab_size.pkl', 'rb') as f:
        vocab_size = pickle.load(f)

    # Load model with exact vocab size used during training
